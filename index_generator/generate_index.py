...
def build_html_tree(current_dir, base_dir):
    ...
    if files:
        ...
        for file in files:
                        ...
                        if meta_match:
                            ...
                            for line in raw_meta.split('\n'):
                                ...
                                if ':' in line_strip and not line_strip.startswith('-'):
                                    ...
                                    if current_key == '知识点':
                                        tags.append(('index-tag', line_strip[1:].strip()))
                                    elif current_key == '模板':
                                        tags.append(('template-tag', line_strip[1:].strip()))
            ...
            tags_html = ""
            if tags:
                tags_html = '<div class="tag-container">' + "".join([f'<span class="{t[0]}">{t[1]}</span>' for t in tags]) + '</div>'
